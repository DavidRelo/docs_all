...
def get_repo_root():
    """获取仓库根目录路径（从全局变量 _WORKTREE_DIR 获取）"""
...
def check_instance_links(mdx_files, config, instance, repo_root, check_remote=False):
    ...
    for file_path in mdx_files:
        ...
        for link_info in links:
            ...
            # 跳过老系统链接（/article/api? 和 /unique-api/ 开头的内部路径）
            if url.startswith('/article/api?') or url.startswith('/unique-api/'):
                continue
...
def _parse_and_set_worktree_dir():
    """从 sys.argv 中提取 --worktree-dir 并设置全局变量（用于 git 模式等提前调用场景）"""
    global _WORKTREE_DIR
    for i, arg in enumerate(sys.argv):
        if arg == '--worktree-dir' and i + 1 < len(sys.argv):
            _WORKTREE_DIR = os.path.abspath(os.path.expanduser(sys.argv[i + 1]))
            break
        elif arg.startswith('--worktree-dir='):
            _WORKTREE_DIR = os.path.abspath(os.path.expanduser(arg.split('=', 1)[1]))
            break
    if not _WORKTREE_DIR or not os.path.isdir(_WORKTREE_DIR):
        print(f'{Fore.RED}错误：请通过 --worktree-dir 指定有效的工作区目录{Style.RESET_ALL}')
        sys.exit(1)
...
''')
    ...
    parser.add_argument('--worktree-dir', metavar='DIR', required=True,
                        help='工作区目录路径（包含 docuo.config.*.json 文件的目录）')

    # 有参数时进入非交互模式
    if len(sys.argv) > 1:
        args = parser.parse_args()
        _WORKTREE_DIR = os.path.abspath(os.path.expanduser(args.worktree_dir))
        if not os.path.isdir(_WORKTREE_DIR):
            print(f'{Fore.RED}工作区目录不存在: {_WORKTREE_DIR}{Style.RESET_ALL}')
            sys.exit(1)
        return run_non_interactive(args)

    # 无参数：由于 worktree-dir 是必填参数，argparse 会自动报错
    parser.parse_args()
